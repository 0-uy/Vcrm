...
import { Patient, Clinic, UserProfile } from './types';
...
export default function App() {
  ...
  const isExpired = clinic?.expiresAt && clinic.expiresAt.toDate() < new Date();
  const isSuspended = (clinic?.status === 'suspended' || isExpired) && profile?.role !== 'superadmin';
  ...
  if (isSuspended) {
    const isActuallyExpired = isExpired && clinic?.status !== 'suspended';
      ...
      <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden p-4">
        ...
        <Card className="max-w-md w-full border-none shadow-2xl rounded-[2.5rem] overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700">
          <CardContent className="p-10 text-center space-y-8">
            <div className={cn(
              "w-24 h-24 rounded-[2rem] flex items-center justify-center mx-auto shadow-inner rotate-3",
              isActuallyExpired ? "bg-orange-500/10 text-orange-500" : "bg-destructive/10 text-destructive"
            )}>
              <ShieldAlert className="w-12 h-12" />
            </div>
            <div className="space-y-3">
              <h1 className="text-3xl font-black tracking-tight">
                {isActuallyExpired ? 'Suscripción Vencida' : 'Cuenta Suspendida'}
              </h1>
              <p className="text-muted-foreground font-medium leading-relaxed">
                {isActuallyExpired 
                  ? `El plan de la clínica "${clinic?.name}" ha vencido. Por favor, contacta al administrador para renovar.` 
                  : `Lo sentimos, el acceso para la clínica "${clinic?.name}" ha sido suspendido temporalmente.`
                }
              </p>
